...
namespace ModuleA.ViewModels
{
    public class ViewAViewModel : BindableBase
    {
        ...
        public DelegateCommand ShowDialogCopyCommand =>
            _showDialogCopyCommand ?? (_showDialogCopyCommand = new DelegateCommand(ShowCopyDialog));

        private void ShowCopyDialog()
        {
            Navigation.ViewLocation(_regionManager, RegionNames.RegisterRegion, ViewNames.ViewC);
            _dialogService.ShowPersonDialog("what the fuck", r =>
            {
                //ex. 2
        ...
        {
             Navigation.ViewLocation(_regionManager, RegionNames.RegisterRegion, ViewNames.ViewB);

        
            _dialogService.ShowMessageDialog("Hello My name is Kim", r =>
            {
                //ex. 2
                if (IsNullOrParameter(r.Parameters))
                {
